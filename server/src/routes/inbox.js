...
import { syncInboundReplies } from '../services/inboxSyncService.js';

router.post('/sync-replies', auth, async (req, res) => {
    try {
        const totalSynced = await syncInboundReplies(req.user.id);
...
router.post('/reply/:threadId', auth, async (req, res) => {
    try {
        ...
        if (targetAccount.connectionType === 'oauth') {
            replyResult = await replyViaOAuth(targetAccount, replyPayload);
        } else {
            const { replyViaScript } = await import('../services/gmailScript.js');
            replyResult = await replyViaScript(targetAccount.scriptUrl, replyPayload);
        }
